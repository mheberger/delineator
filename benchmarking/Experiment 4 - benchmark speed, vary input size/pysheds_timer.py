...
Matthew Heberger, Nov 2023

For experiment #4 where I am investigating the relationship between raster 
size and delineation speed.
...
def timer(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        end_time = time.time()
        execution_time = sigfig.round(end_time - start_time, 3)
...
def main():
    ...
    gages_df = pandas.read_csv(INFILE, header=0,
                               dtype={'id': 'str', 'lat': 'float', 'lng': 'float',  'basin': 'int'})

    # We'll record the time in this field
    gages_df['lat_snap'] = np.nan
    gages_df['lng_snap'] = np.nan
    gages_df['delineation_time'] = np.nan
    ...
    # Iterate over gages
    for i in range(0, n):
        if gages_df['basin'].iloc[i] != megabasin:
            megabasin = gages_df['basin'].iloc[i]
            ...
            print(f"Reading accum raster {accum_fname}")
            elapsed_time, acc = read_accum(accum_fname, grid)

            # Read the flow direction grid
            print(f"Reading flow direction raster {fdir_fname}")
        ...
        try:
            ...
            gages_df.loc[i, 'lat_snap'] = y_snap
            gages_df.loc[i, 'lng_snap'] = x_snap

            # Delineation with pysheds catchment() command
            catch = grid.catchment(x=x_snap, y=y_snap, fdir=fdir, xytype='coordinate')

            # This step was critical to getting a clean export
            grid.clip_to(catch)
    ...
    gages_df.to_csv(OUTFILE, index=False)


@timer
def read_fdir(fdir_fname, grid):
    return grid.read_raster(fdir_fname, nodata=247)


@timer
def read_accum(accum_fname, grid):
    return grid.read_raster(accum_fname, nodata=0)
