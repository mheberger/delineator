...
def main():
    ...
    acc = grid.read_raster(accum_fname)
    ...
    for i in range(0, n):
        ...
        # Snap pour point
        lng_snap, lat_snap = grid.snap_to_mask(streams, (lng, lat))
        ...
        catch = grid.catchment(x=lng_snap, y=lat_snap, fdir=fdir, xytype='coordinate')
        ...
        grid.to_raster(catch_view, tiff_name, dtype=np.uint8,
                       blockxsize=16, blockysize=16, compress='deflate')
    ...
    p1 = gpd.GeoSeries(gpd.points_from_xy(gages_df['lng'], gages_df['lat']), crs='EPSG:4326').to_crs(3057)
    p2 = gpd.GeoSeries(gpd.points_from_xy(gages_df['lng_snapped'], gages_df['lat_snapped']),
                       crs='EPSG:4326').to_crs(3057)

    gages_df['dist_m'] = round(p1.distance(p2, align=False))

    # Write the timer data to a csv file
    gages_df.to_csv(f'pysheds_results.csv')
